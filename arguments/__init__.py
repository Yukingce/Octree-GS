...
class ModelParams(ParamGroup): 
    def __init__(self, parser, sentinel=False):
        ...
        self.base_layer = -1 # -1 # -1(adaptive) or 10 (default) or 0 ~
    ...
    def extract(self, args):
        ...
        g.model_path = os.path.join(g.model_path, time.strftime("%Y-%m-%d_%H-%M-%S_", g.time) + g.desc)
        os.makedirs(g.model_path, exist_ok=True)
        with open(os.path.join(g.model_path, 'config.json'), 'w') as f:
            json.dump(vars(args), f, indent=2, separators=(',', ':'))
        return g
...
class OptimizationParams(ParamGroup):
    def __init__(self, parser):
        ...
        self.coarse_iter = 15000 # 10000
        self.coarse_factor = 1.5
        self.update_interval = 100
        self.update_until = 35000 # 25000
