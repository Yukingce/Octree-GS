...
class GaussianModel:
    ...
    def training_setup(self, training_args):
        ...
        self.offset_gradient_accum = torch.zeros((self.get_anchor.shape[0]*self.n_offsets, 1), device="cuda") # 每个神经高斯的梯度计数
        ...
        '''
        用于生成学习率调度器（LR scheduler），返回一个以训练步数为输入、返回当前学习率的函数。
        lr_delay_steps: 延迟步数（默认 0）
        lr_delay_mult: 延迟期间的学习率缩放因子（默认 1.0）

        对数线性插值（指数衰减）：
        step=0 时为 lr_init
        step=max_steps 时为 lr_final
        中间按对数空间线性插值，等价于指数衰减
        延迟启动（若 lr_delay_steps > 0）：
        前 lr_delay_steps 步使用较小的学习率（lr_init * lr_delay_mult）
        之后平滑过渡到正常学习率（使用正弦函数平滑）
        '''
    ...
    # '''使用原版3dgs的opacity的优化管线，不使用mlp预测'''
