    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        if masks_folder is not None:
            if os.path.exists(os.path.join(masks_folder, os.path.splitext(extr.name)[0] + ".png")):
                mask_path = os.path.join(masks_folder, os.path.splitext(extr.name)[0] + ".png")
            else:
                mask_path = None
        else:
            mask_path = None

        cam_info = CameraInfo(uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image_id=image_id,
                              image_path=image_path, image_name=image_name,  mask_path=mask_path, width=width, height=height)
...
def readColmapSceneInfo(path, images, eval, ds, masks_folder, llffhold=8):
    try:
    ...
    reading_dir = images if images is not None else f"images"

    # wy
    masks_dir = os.path.join(path, masks_folder) if masks_folder in os.listdir(path) else None
    ...
    cam_infos = sorted(cam_infos_unsorted.copy(), key = lambda x : x.image_id)

    test_num = min(max(20, int(len(cam_infos) * 0.02)), 40)
    sample_interval = int(len(cam_infos) / test_num)

    if eval:
        # train_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold != 0]
        # test_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % llffhold == 0]
        train_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % sample_interval != 0]
        test_cam_infos = [c for idx, c in enumerate(cam_infos) if idx % sample_interval == 0]
