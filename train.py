...
def training(dataset, opt, pipe, dataset_name, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, wandb=None, logger=None, ply_path=None):
    ...
    for viewpoint in tqdm(viewpoint_stack, "Camera image is initializing! image deactivate_percent: {:.2f}".format(dataset.deactivate_percent)):
        activate = False if uniform(0, 1) < dataset.deactivate_percent else True
        viewpoint.init_gt_image(activate=activate)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if viewpoint_cam.active:
            gt_image = viewpoint_cam.original_image
            mask = viewpoint_cam.valid_mask

        else:
            gt_image, mask = viewpoint_cam.get_deactivated_image()


        Ll1 = l1_loss(image, gt_image, mask)

        ssim_loss = (1.0 - ssim(image, gt_image, mask=mask))
        if scaling.shape[0] > 0:
            scaling_reg = scaling.prod(dim=1).mean() # .prod(dim=1) 表示 在第 1 个维度上求积。也就是说，对每一行（或样本）内的所有元素相乘。
        else:
        ...
        # else:
        #     mlp_opacity = opacity.mean()
        #     loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * ssim_loss + 0.01 * scaling_reg + 0.05 * (1.0 - mlp_opacity)
...
def training_report(tb_writer, dataset_name, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, wandb=None, logger=None):
    ...
    if iteration in testing_iterations:
        ...
        if len(train_cams) > 50:
            num = min(max(50, int(len(train_cams) * 0.02)), 100)
            sample_interval = int(len(train_cams) / num)
            config = {'name': 'train', 'cameras': [c for idx, c in enumerate(train_cams) if idx % sample_interval == 0]}
        else:
            config = {'name': 'train', 'cameras': train_cams}
        ...
        if config['cameras'] and len(config['cameras']) > 0:
            ...
            for idx, viewpoint in enumerate(config['cameras']):
                ...
                if viewpoint.active:
                    original_image = viewpoint.original_image
                    mask = viewpoint.valid_mask
                else:
                    original_image, mask = viewpoint.get_deactivated_image()
                gt_image = torch.clamp(original_image.to("cuda"), 0.0, 1.0)
                ...
                psnr_test += psnr(image, gt_image, mask).double()
                ssim_test += ssim(image, gt_image, mask=mask).mean().double()
                if mask is not None:
                    lpips_test += lpips_fn(image * mask, gt_image * mask).mean().double().detach()
                else:
                    lpips_test += lpips_fn(image, gt_image).mean().double().detach()

            psnr_test /= len(config['cameras'])
            ssim_test /= len(config['cameras'])
            lpips_test /= len(config['cameras'])
            l1_test /= len(config['cameras'])
            logger.info("\n[ITER {}] Evaluating {}: L1 {:>12.7f} PSNR {:>12.7f} SSIM {:>12.7f} LPIPS {:>12.7f}".format(
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train=False, skip_test=False, wandb=None, tb_writer=None, dataset_name=None, logger=None):
    with torch.no_grad():
        ...
        if not skip_train:
            ...
            for viewpoint in tqdm(viewpoints, "Camera image is initializing!"):
                viewpoint.init_gt_image(activate=True)

            t_train_list, visible_count = render_set(dataset.model_path, "train", scene.loaded_iter,
                                                     viewpoints, gaussians, pipeline, background)
            train_fps = 1.0 / torch.tensor(t_train_list[5:]).mean()
            # logger.info(f'Train FPS: \033[1;35m{train_fps.item():.5f}\033[0m')
            if wandb is not None:
                wandb.log({"train_fps": train_fps.item(), })
...
if __name__ == "__main__":
    ...
    # training
    training(model_params, op.extract(args), pp.extract(args), dataset,  args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, wandb, logger)
    if args.warmup:
        logger.info("\n Warmup finished! Reboot from last checkpoints")
        new_ply_path = os.path.join(args.model_path, f'point_cloud/iteration_{args.iterations}', 'point_cloud.ply')
        training(model_params, op.extract(args), pp.extract(args), dataset,  args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, wandb=wandb, logger=logger, ply_path=new_ply_path)
    ...
    logger.info(f'\nStarting Rendering~')
    if args.eval:
        visible_count = render_sets(model_params, -1, pp.extract(args), skip_train=True, skip_test=False, wandb=wandb, logger=logger)
    else:
        visible_count = render_sets(model_params, -1, pp.extract(args), skip_train=False, skip_test=True, wandb=wandb, logger=logger)
    logger.info("\nRendering complete.")

    # calc metrics
    logger.info("\n Starting evaluation...")
    eval_name = 'test' if args.eval else 'train'
    evaluate(model_path, eval_name, visible_count=visible_count, wandb=wandb, logger=logger)
    logger.info("\nEvaluating complete.")
